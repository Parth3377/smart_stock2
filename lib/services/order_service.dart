...
//  Delegates Firestore operations to FirestoreService.
// ════════════════════════════════════════════════════════════════════
...
class OrderService {
  ...
  // Local cache for instant UI (before Firestore responds)
  ...
  /// Add order locally + save to Firestore in background
  static void addOrder(OrderModel order) {
    orders.insert(0, order);
    FirestoreService.instance.saveOrder(order).catchError((_) {});
  }

  /// Stream orders for current user from Firestore
  static Stream<List<OrderModel>> streamUserOrders() =>
      FirestoreService.instance.streamMyOrders();

  /// Stream order count from Firestore
  static Stream<int> streamOrderCount() =>
      FirestoreService.instance.streamMyOrderCount();
