...
//
// // ════════════════════════════════════════════════════════════════════
// //  1. AUTH PROVIDER (unchanged — logic is in FirebaseAuthProvider)
// // ════════════════════════════════════════════════════════════════════
// class AdminAuthProvider extends ChangeNotifier {
//   bool _isAdmin = false;
//   String _adminName = '';
...
//
// // ════════════════════════════════════════════════════════════════════
// //  3. DASHBOARD PROVIDER — Firestore real-time
// // ════════════════════════════════════════════════════════════════════
// class AdminDashboardProvider extends ChangeNotifier {
//   AdminLoadState state = AdminLoadState.idle;
...
//     try {
//       // First load
//       stats = await DashboardService.instance.fetchStats();
//       state = AdminLoadState.loaded;
//       notifyListeners();
//
//       // Subscribe to live updates
//       _sub?.cancel();
//       _sub = DashboardService.instance.streamDashboard().listen((s) {
//         stats = s;
//         notifyListeners();
//       });
//     } catch (e) {
...
//   }
//
//   // Keep chart helpers so existing UI doesn't break
//   List<AdminMonthData> get monthlyRevenue => [];
//   List<AdminMonthData> get monthlyCost => [];
//   List<AdminCategoryData> get categoryData => [];
// }
//
...
//
// // ════════════════════════════════════════════════════════════════════
// //  4. PRODUCTS PROVIDER — Firestore real-time stream
// // ════════════════════════════════════════════════════════════════════
// class AdminProductsProvider extends ChangeNotifier {
//   AdminLoadState state = AdminLoadState.idle;
//   List<AdminProduct> _all = [];
...
//
//     _sub?.cancel();
//     _sub = ProductService.instance.streamProducts().listen(
//           (docs) {
//         _all = docs.map(_docToProduct).toList();
//         state = AdminLoadState.loaded;
//         notifyListeners();
//       },
//       onError: (e) {
...
//
// // ════════════════════════════════════════════════════════════════════
// //  5. ORDERS PROVIDER — Firestore real-time
// // ════════════════════════════════════════════════════════════════════
// class AdminOrdersProvider extends ChangeNotifier {
//   AdminLoadState state = AdminLoadState.idle;
//   List<AdminOrder> _all = [];
...
//   List<AdminOrder> get allOrders => _all;
//
//   Future<void> add(AdminOrder order) async {
//     _all.add(order);
//     notifyListeners();
//   }
//
...
//
//     _sub?.cancel();
//     _sub = OrderService.instance.streamAllOrders().listen(
//           (docs) {
//         _all = docs.map(_docToOrder).toList();
//         state = AdminLoadState.loaded;
//         notifyListeners();
//       },
//     );
...
//
//   Future<void> updateStatus(String id, String status) async {
//     await OrderService.instance.updateOrderStatus(id, status);
//   }
//
//   void setSearch(String q) { searchQuery = q; page = 0; notifyListeners(); }
//   void setStatus(String s) { statusFilter = s; page = 0; notifyListeners(); }
...
//
// // ════════════════════════════════════════════════════════════════════
// //  6. SUPPLIERS PROVIDER — Firestore real-time
// // ════════════════════════════════════════════════════════════════════
// class AdminSuppliersProvider extends ChangeNotifier {
//   AdminLoadState state = AdminLoadState.idle;
//   List<AdminSupplier> _all = [];
...
//
//     _sub?.cancel();
//     _sub = SupplierService.instance.streamSuppliers().listen(
//           (docs) {
//         _all = docs.map(_docToSupplier).toList();
//         state = AdminLoadState.loaded;
//         notifyListeners();
//       },
//     );
...
//
//   Future<void> add(AdminSupplier s) async {
//     await SupplierService.instance.addSupplier(
...
//
//   Future<void> delete(String id) async {
//     await SupplierService.instance.deleteSupplier(id);
//   }
//
//   void setSearch(String q) { searchQuery = q; notifyListeners(); }
//
...
//
// // ════════════════════════════════════════════════════════════════════
// //  7. TRANSFERS PROVIDER — Firestore real-time
// // ════════════════════════════════════════════════════════════════════
// class AdminTransferProvider extends ChangeNotifier {
//   AdminLoadState state = AdminLoadState.idle;
//   List<AdminTransfer> _all = [];
...
//
//     _sub?.cancel();
//     _sub = StockTransferService.instance.streamTransfers().listen(
//           (docs) {
//         _all = docs.map(_docToTransfer).toList();
//         state = AdminLoadState.loaded;
//         notifyListeners();
//       },
//     );
...
//
//   Future<void> add(AdminTransfer t) async {
//     await StockTransferService.instance.createTransfer(
//       productId: '',
...
//       email = data['email'] as String? ?? 'admin@smartstock.com';
//       role = data['role'] as String? ?? 'Super Admin';
//       id = doc.id;
//       notifyListeners();
//     }
//   }
...
//     if (name != null) { this.name = name; updates['name'] = name; }
//     if (email != null) { this.email = email; updates['email'] = email; }
//     if (updates.isNotEmpty) {
//       await _db.collection('admins').doc(id).update(updates);
//     }
//     notifyListeners();
//   }
// }